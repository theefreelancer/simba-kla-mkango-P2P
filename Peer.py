""" This script contains both the server and client functionality, and therefore we don't separate
client and server scripts. With this script a peer is able to:-
1. Join the network
2. Leave the network
3. Upload a resource
4. Search for a resource
5. Print Hashing table
6. View Predecessor and successor nodes
"""
...
import os
import threading
import pickle #for file transfer
import sys
import time
from collections import OrderedDict
# Default IP, PORT and buffer size if command line arguments are not given
IP = "10.10.10.71"
PORT = 3500
buffer = 4096
...
class Node:
    ...
    def asAClientThread(self):
        ...
        userChoice = input()
        if userChoice == "1":
            ip = input("New connection\nEnter Node IP to connect: ")
            port = input("Enter port number: ")
            self.sendJoinRequest(ip, int(port))
        elif userChoice == "2":
            self.leaveNetwork()
        elif userChoice == "3":
            filename = input("Enter resource name to upload: ")
            fileID = getHash(filename)
            recvIPport = self.getSuccessor(self.succ, fileID)
            self.uploadFile(filename, recvIPport, True)
        elif userChoice == "4":
            filename = input("Enter resource name you are searching for: ")
    ...
    def downloadFile(self, filename):
        ...
        if fileData == b"NotFound":
            ...
            print("Receiving resource:", filename)
    ...
    def printMenu(self):
        print("Welcome to Crane Cloud (CC)\n1. Join Network\n2. Leave Network\n3. Upload Resource\n4. Search Resource")
        print("5. Print Hashing Table\n6. Print my predecessor and successor nodes")
